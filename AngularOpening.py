...
OpeningAngle = numpy.rad2deg(
    numpy.arctan((options.FOV / 2) / options.Distance)) * 2
...
for Displacement in (0,
    ...
    cmoscolor = 'b'
    plt.plot((0, 0),
        (Displacement + 3, Displacement - 3), linewidth='5', color=cmoscolor)
    ...
    plt.plot(
        ...
        [Displacement + (options.FOV / 2), Displacement - (options.FOV / 2)],
        linewidth='6', color=screencolor)
    screencolor = 'g'
    plt.plot(
        [options.Distance, options.Distance],
        [Displacement + (options.FOV / 2), Displacement - (options.FOV / 2)],
        linewidth='4', color=screencolor)
...
plt.title('Angular opening: ' + str(round(OpeningAngle, 2)) +
    '\nFOV size: ' + str(options.FOV) + ' mm (including overlap of ' +
    str(options.Overlap) + ' %)\nWorking Distance: ' +

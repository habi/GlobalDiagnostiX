...
parser.add_option('-d', dest='Distance', type='float',
    default = 134, metavar='123',
    help='Scintillator-CMOS distance [mm]. Default = %default mm')
parser.add_option('-f', dest='FOV', type='float',
    default = 450/3, metavar='430',
    help='Desired field of view [mm]. Default = %default mm')
parser.add_option('-o', dest='Overlap', type='float',
    default = 2, metavar='16',
    help='Overlap between the images [%]. Default = %default %')
parser.add_option('-p', dest='ParaventLength', type='float',
    default = 100, metavar='123',
    help='Length of the paravents. Default = %default mm')
...
parser.add_option('-s', dest='SaveImage',
...
if options.Distance == None or options.FOV == None:
    parser.print_help()
    print ''
    print 'Example:'
...
OpeningAngle = numpy.rad2deg(numpy.arctan((options.FOV / 2) / options.Distance)) * 2
print round(OpeningAngle,1), 'degrees'

plt.figure(figsize=(5,15))
for Displacement in (0, -options.FOV / (1+options.Overlap/100), options.FOV/(1+options.Overlap/100)):
    # Central axis
    plt.axhline(Displacement, color='k',linestyle='--')

    # CMOS
    cmoscolor = 'b'
    plt.plot( (0, 0), (Displacement + 3, Displacement -3),linewidth='5',color=cmoscolor)

    # Lens
    rect = Rectangle((options.BackFocalLength, Displacement-14/2), options.LensLength, 14,
        facecolor="#aaaaaa")
    plt.gca().add_patch(rect)

    # Opening angle, based on CMOS
    wedge = Wedge((0,Displacement), options.Distance * 0.309, -OpeningAngle/2, OpeningAngle/2, fill=True, color='r', alpha=0.125)
    plt.gca().add_patch(wedge)
    plt.plot((0,options.Distance), (Displacement, Displacement + options.FOV/2), color='k', linestyle='--', alpha=0.25)
    plt.plot((0,options.Distance), (Displacement, Displacement - options.FOV/2), color='k', linestyle='--', alpha=0.25)
    ...
    plt.plot(
        ...
        [Displacement+(options.FOV/2),Displacement-(options.FOV/2)],linewidth='6',color=screencolor)
    screencolor = 'g'
    plt.plot(
        [options.Distance,options.Distance],
        [Displacement+(options.FOV/2),Displacement-(options.FOV/2)],linewidth='4',color=screencolor)

    # FOV drawn from center of lens
    beamcolor = 'r'
    plt.plot([options.BackFocalLength+options.LensLength,options.Distance],[Displacement,Displacement + options.FOV/2],beamcolor)
    plt.plot([options.BackFocalLength+options.LensLength,options.Distance],[Displacement,Displacement - options.FOV/2],beamcolor)
    ...
    plt.plot(
        ...
        [Displacement -(options.FOV / (1+options.Overlap/100)/2),Displacement-(options.FOV / (1+options.Overlap/100)/2)],linewidth='5',color=paraventcolor)
...
plt.title('Angular opening: ' + str(round(OpeningAngle,2)) + '\nFOV size: ' +\
    str(options.FOV) + ' mm (including overlap of ' + str(options.Overlap) + ' %)\nWorking Distance: ' +\
    str('%.2f' % options.Distance) + ' mm\nParavent length: ' + str('%.2f' % options.ParaventLength) + ' mm')
plt.xlabel('Distance [mm]')
plt.xlim([-10, options.Distance + 10])
#~ plt.ylim([-4*options.FOV, 4*options.FOV])
plt.axis('equal')

if options.SaveImage:
    SaveName = 'Paravents_' +str(str('%.2f' % OpeningAngle)) + '_wd_' +\
        str('%.2f' % options.Distance) + 'mm_FOV_' + str('%.2f' % options.FOV) + 'mm'

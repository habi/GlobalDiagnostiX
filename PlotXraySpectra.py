...
Mean = [ np.round(double(open(FileName).readlines()[6].split()[3]),decimals=2) for FileName in Spectra ]

for i in range(len(Spectra)):
	plt.plot( Data[i][:,0], Data[i][:,1], label= str(Energy[i]) + 'kV, Mean=' + str(Mean[i]) +'keV' )

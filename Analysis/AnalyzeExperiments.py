...
# Sort according to some defined value
sortedLogs = [x for (y,x) in sorted(zip(Mean,LogFiles), reverse=True)]
sortedValue = [x for (y,x) in sorted(zip(Mean,Mean), reverse=True)]
sortedIDs = [x for (y,x) in sorted(zip(Mean,ExperimentID), reverse=True)]

plt.figure(figsize=[16,9])
for c, i in enumerate(sortedLogs):
    plt.subplot(2,len(LogFiles),c +1)
    Image = i[:-len('analysis.log')] + 'image.corrected.stretched.png'
    plt.imshow(plt.imread(Image), cmap='bone')
    plt.title(' '.join([str(sortedIDs[c]), 'Mean', str(sortedValue[c])]))
    plt.axis('off')

    plt.subplot(2,len(LogFiles),c + 1 + len(LogFiles))
    Image = LogFiles[c][:-len('analysis.log')] + 'image.corrected.stretched.png'
    plt.imshow(plt.imread(Image), cmap='bone')
    plt.title(' '.join([str(ExperimentID[c]), 'Mean', str(Mean[c])]))
    plt.axis('off')

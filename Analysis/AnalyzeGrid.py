...
for Counter, SelectedExperiment in enumerate(Experiment):
    ...
    if decide:
        ...
        plt.figure(' '.join([str(Counter + 1) + '/' + str(len(Experiment)),
            '| Decision |', Scintillator[Counter], '|', Sensor[Counter], '|',
            Lens[Counter], '|', ExperimentID[Counter], '|', str(SDD[Counter]),
            'mm | git version', git_hash]),
            figsize=(16, 12))
    ...
    plt.figure(' '.join([str(Counter + 1) + '/' + str(len(Experiment)),
        ...
        Sensor[Counter], '|', Lens[Counter], '|', ExperimentID[Counter], '|',
        str(SDD[Counter]), 'mm | git version', git_hash]),
        figsize=(16, 9))
    plt.subplot(221)

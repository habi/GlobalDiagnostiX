...
Scintillator = [linecache.getline(i, 9).split(':')[1].strip() for i in
                LogFiles]
...
Exposuretime = [float(linecache.getline(i, 18).split(':')[1].split('ms')[0].strip()) for i in LogFiles]
...
for Counter, SelectedExperiment in enumerate(Experiment):
    ...
    # Load image
    ResolutionFileName = os.path.join(os.path.dirname(Experiment[Counter]),
                                      ExperimentID[Counter] +
                                      '.resolution.png')
    decide = True
    if os.path.isfile(ResolutionFileName):
        ...
        plt.figure(' '.join([str(Counter + 1) + '/' + str(len(Experiment)),
                             ...
                             Sensor[SelectedExperiment], '|',
                             Lens[SelectedExperiment], '|',
                             ExperimentID[SelectedExperiment], '|',
                             str(SDD[SelectedExperiment]), 'mm | git version',
                             git_hash]), figsize=(20, 12))
    ...
    if decide:
        ...
        SelectionFileName = os.path.join(os.path.dirname(Experiment[Counter]),
                                         ExperimentID[Counter] +
                                         '.image.corrected.stretched.png')
        SelectionImage = plt.imread(SelectionFileName)
        plt.figure(' '.join([str(Counter + 1) + '/' + str(len(Experiment)),
                             '| Decision |', Scintillator[Counter], '|',
                             Sensor[Counter], '|', Lens[Counter], '|',
                             ExperimentID[Counter], '|', str(SDD[Counter]),
    ...
    logfile.info(os.path.join(os.path.dirname(Experiment[Counter]),
                              ExperimentID[Counter] + '.image.corrected.png'))
    logfile.info(os.path.join(os.path.dirname(Experiment[Counter]),
                              ExperimentID[Counter] +
                              '.image.corrected.stretched.png'))
    logfile.info(80 * '-')
    ...
    plt.figure(' '.join([str(Counter + 1) + '/' + str(len(Experiment)),
                         '| Originals & Histograms |', Scintillator[Counter],
                         '|', Sensor[Counter], '|', Lens[Counter], '|',
                         ExperimentID[Counter], '|', str(SDD[Counter]),
                         'mm | git version', git_hash]), figsize=(16, 9))
    plt.subplot(221)
    plt.imshow(OriginalImage, cmap='bone', interpolation='bicubic')
    plt.title(' '.join([ExperimentID[Counter] +
                        '.image.corrected.png (' + str(OriginalImage.shape[1]),
                        'x', str(OriginalImage.shape[0]), 'px)']))
    plt.axis('off')
    plt.subplot(222)
    plt.imshow(StretchedImage, cmap='bone', interpolation='bicubic')
    plt.title(' '.join([ExperimentID[Counter] +
                        '.image.corrected.stretched.png (' +
                        str(StretchedImage.shape[1]), 'x',
                        str(StretchedImage.shape[0]), 'px)']))
    # Histograms
    bins = 256
    plt.subplot(223)
    plt.hist(OriginalImage.flatten(), bins, histtype='stepfilled', normed=True)
    # plt.xlim([0, 0.95])
    plt.subplot(224)
    ...
    plt.subplot(221)
    plt.title(' '.join([ExperimentID[Counter] + '.image.corrected.png with',
                        str(BigROISize[0]), 'x', str(BigROISize[1]),
                        'px ROI']))
    currentAxis = plt.gca()
    BigROI = currentAxis.add_patch(Rectangle((xmin, ymin), xmax - xmin,
                                             ymax - ymin, facecolor='red',
    ...
    fig = plt.figure(' '.join([str(Counter + 1) + '/' + str(len(Experiment)),
                               '| Result |', Scintillator[Counter], '|',
                               Sensor[Counter], '|', Lens[Counter], '|',
                               ExperimentID[Counter], '|', str(SDD[Counter]),
                               'mm | git version', git_hash]), figsize=(16, 9))
    plt.suptitle(' '.join([Scintillator[Counter], '|', Sensor[Counter], '|',
                           Lens[Counter], '|', str(SDD[Counter]),
                           'mm | version', git_hash]))
    ...
    SelectedLines = [line for line in
                     [CroppedImage[int(round(height)), xxmin - pad:xxmax + pad]
                      for height in SelectedHeight]]
    SelectedLinesStretched = [line for line in
                              [CroppedImageStretched[int(round(height)),
                               xxmin - pad:xxmax + pad] for height in
                               SelectedHeight]]
    ...
    tellme(' '.join([str(BigROISize[0]), 'x', str(BigROISize[1]),
                     'px ROI\nLocation of', str(steps),
                     'lines from plots below']))
    ...
    plt.title(' '.join(['Brightness in the green', str(LineROISize[0]), 'x',
                        str(LineROISize[1]), 'px ROI.\nTop: original image, '
                                             'bottom: contrast stretched '
                                             'image. Color: lines from above '
                                             'right, Black: Mean of',
                        str(steps), 'lines']))
    ...
    SaveName = os.path.join(os.path.dirname(Experiment[Counter]),
                            ExperimentID[Counter] + '.resolution.png')

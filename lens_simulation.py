...
print CMOSSize[1]
print FOVSize[1]
print FOVSize[1] / CMOSSize[1]
print FOVSize[0] / CMOSSize[0]
...
print 80 * '-'
print 'For a Scintillator FOV of', round(FOVSize[0]), 'x', round(FOVSize[1]),\
    'mm and a CMOS size of', round(CMOSSize[0]), 'x', round(CMOSSize[1]), \
    'mm, we get a demagnification of', round(Magnification[0]), 'x', \
    round(Magnification[1])
...
# ------------------------------------
LensPosition = np.array([0])  # lens positions
FocalLength = np.array([3])  # lens focal length
...
plt.plot((-CMOSPosition, -CMOSPosition), (-CMOSSize[0] / 2, CMOSSize[0] / 2),
         "b")
plt.plot((FOVPosition, FOVPosition), (-FOVSize[0] / 2, FOVSize[0] / 2), "g")

# Draw Lens(es)
for i in range(np.size(LensPosition)):
    add_lens(LensPosition[i], FocalLength[i], 5, "L" + str(i))

plt.savefig("lens_simulation.png")
plt.show()
exit()

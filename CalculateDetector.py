...
NumericalApertureAverage = \
    integrate.quad(lambda x: np.arctan(FlatPanelPixelSize / ( 2 * x )), 0.01, 1)[0]
...
plt.title('Angular opening: ' + str(
    '%.2f' % options.OpeningAngle) + ', Screen size: ' + \
          str('%.2f' % options.FOV) + 'cm, Working Distance: ' + \
          str('%.2f' % round(WorkingDistance, 2)) + 'cm\n' + \
          'Scintillator Efficiency: ' + str(
    round(ScreenOutput, 2) * 100) + '%, ' + \
          'Lens transmission: ' + str(
    round(LensTransmission, 2) * 100) + '%, ' + \
...
Scintillator = Rectangle((-Thickness, options.FOV / 2), Thickness, -options.FOV,
...
for FStop in [0.5, 0.8, 1, 1.2, 1.4, 2]:
    plt.plot(Magnification,
             Magnification / ( 2 * FStop * ( 1 + Magnification )),
...
plt.text(1 / Demagnification + 0.25, 0.8,
         'Our calculated\nDemagnification: ' + str(
             Demagnification) + 'x=' + str(round(1 / Demagnification, 3)))
...
FilterMaterial = [str(open(FileName).readlines()[9].split()[1]) for FileName in
                  Spectra]
FilterThickness = [int(open(FileName).readlines()[9].split()[2]) for FileName in
                   Spectra]
...
if options.Output:
    ...
    try:
        ...
        print 'Directory', os.path.join(os.getcwd(),
                                        Prefix), 'already exists, did not create it...'
    print

    # We should probably do something more clever with "print "%10.4f" % options" than the stuff below
    SaveName = Prefix + str(options).replace('{', '_').replace('}', '').replace(
        "'", '').replace(': ', '_').replace(', ', '-').replace('-Output_True',
                                                               '').replace(
        '9999999999999',
    ...
    logfile = open(os.path.join(Prefix, ''.join([SaveName, '.txt'])),
                   'w')  # open the result file in write mode
    ...
    print '  -', int(
        round(PhotonsAfterScintillator)), 'photons with a wavelength' \
                                          'of', int(
        Wavelength * 1e9), 'nm (or', round(PhotonEnergyeV, 3), 'eV).'

    print '  -', int(round(PhotonsAfterLens)), 'of these photons (' + \
                                               str(
                                                   LensTransmission * 100) + ' %) will arrive at the sensor'
    ...
    print 'To achieve', options.LinePairs, 'lp/mm on a', options.FOV, \
        'cm scintillator, we need a sensor with', round(
        int(PixelsNeeded) ** 2 / 1e6, 1), \
        'Mpx (' + str(int(PixelsNeeded)) + 'x' + str(int(PixelsNeeded)), \
        'px), which results in pixels with a physical size of', \
        round(SensorPixelSize * 1000,
              2), 'um on a', options.SensorSize, 'cm sensor.'

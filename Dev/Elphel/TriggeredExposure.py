...
- Set exposure time, either via wget or PHP call to something along the lines
    of snapfull.php
- Spin up anode with a 4V signal over one pin
- Wait for camera signaling to be ready
...
# number it actually is. Pins 2 and 4 output 5V, Pin 6 is Ground.
if options.Test is False:
    GPIO.setmode(GPIO.BOARD)
    GPIO.setup(AnodePin, GPIO.OUT)
# Inform the user what will happen
print 'As soon as you confirm with [Enter], I will trigger everything:'
...
if options.Test:
    ...
    while True:
        # Wait for camera to signal readyness on the InputPin
...
ImageName = time.time()  # current epoch time as filename, avoiding duplicates
if options.Test:
...
print
# 38 = 80 / 2 - len('Done') / 2, 80 being the line length
print 38 * ' ' + 'DONE!'
print
